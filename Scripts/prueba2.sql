...
WHERE dp.DP_FK_PEDIDO=pe.PD_ORDEN AND dp.DP_FK_PRODUCTO=p.PR_CODIGO AND df.DF_FK_PEDIDO=dp.DP_FK_PEDIDO AND dp.DP_ID=df.DF_FK_DETPED AND df.DF_FK_FACT_T=ft.FT_CODIGO AND ft.FT_CODIGO = 2 AND ft.FT_FK_CLIENTE=cv.CV_ID AND ft.FT_FK_TIENDA=ti.TI_ID AND cv.CV_FK_CIUDAD=ecc.EC_ID AND ecc.EC_FK_ESTADO=ece.EC_ID AND ecc.EC_FK_PAIS=pais.PA_ID AND ece.EC_FK_PAIS=pais.PA_ID
GROUP BY ti.TI_ID, ft.FT_CODIGO,ft.FT_FECHA,cv.CV_NOMBRE,cv.CV_NOMBRE_S,cv.CV_APELLIDO,cv.CV_APELLIDO_S,ecc.EC_NOMBRE,ece.EC_NOMBRE,PA_NOMBRE,PR_CODIGO, PR_NOMBRE, PR_PRECIO_USD, ft.FT_MONTO_USD;
